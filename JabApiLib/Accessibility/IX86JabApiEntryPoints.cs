...
namespace Tct.Java.Accessibility
{
	interface IX86JabApiEntryPoints
	{
		...
		bool GetAccessibleActions(int vmId, CustomPtr accessibleContext, out AccessibleActions actions);
		bool DoAccessibleActions(int vmId, CustomPtr accessibleContext, ref AccessibleActionsToDo accessibleActionsToDo, out int failure);
		bool SetTextContents(Int32 vmId, CustomPtr accessibleContext, string text);
		bool RequestFocus(int vmId, CustomPtr accessibleContext);
	}
}
