...
namespace Tct.Java.Accessibility
{
	class JabApiEntryPointsX86: IX86JabApiEntryPoints
	{
		...
		[DllImport(WinAccessBridgeDll, SetLastError = true, ThrowOnUnmappableChar = true, CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Auto)]
		public static extern bool getAccessibleActions(int vmId, CustomPtr accessibleContext, [Out] AccessibleActions actions);

		[DllImport(WinAccessBridgeDll, SetLastError = true, ThrowOnUnmappableChar = true, CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Auto)]
		public static extern bool doAccessibleActions(int vmId, CustomPtr accessibleContext, ref AccessibleActionsToDo actionsToDo, out int failure);

		[DllImport(WinAccessBridgeDll, SetLastError = true, ThrowOnUnmappableChar = true, CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Auto)]
		public static extern bool setTextContents(int vmId, CustomPtr ac, string text);

		[DllImport(WinAccessBridgeDll, SetLastError = true, ThrowOnUnmappableChar = true, CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Auto)]
		public static extern bool requestFocus(int vmId, CustomPtr ac);
		...
		public bool GetAccessibleActions(int vmId, CustomPtr accessibleContext, out AccessibleActions actions)
		{
			actions = new AccessibleActions();
			return getAccessibleActions(vmId, accessibleContext, actions);
		}

		public bool DoAccessibleActions(int vmid, CustomPtr accessibleContext, ref AccessibleActionsToDo actionsToDo, out int failure)
		{
			return doAccessibleActions(vmid, accessibleContext, ref actionsToDo, out failure);
		}

		public bool SetTextContents(int vmId, CustomPtr ac, string text)
		{
			return setTextContents(vmId, ac, text);
		}

		public bool RequestFocus(int vmId, CustomPtr accessibleContext)
		{
