...
namespace JavaAccessibility
{
	public class JabApiController
	{
		...
		public AccessibleWrapper GetAccessibleContextAt(Point p)
		{
			foreach (var process in Process.GetProcesses())
			{
				try
				{
					...
					{
						var aw = GetContextFromHwnd(process.MainWindowHandle);
						var rect = new Rectangle(aw.Location, new Size(aw.Width, aw.Height));
						if (rect.Contains(p))
						{
							AccessibleContextInfo info = jabApi.GetAccessibleContextAt(aw.vmId, null, p.X, p.Y, out dynamic pointer);
							return new AccessibleWrapper(aw.vmId, pointer)
							{
								Description = info.description,
								Name = info.name,
								Role = info.role,
