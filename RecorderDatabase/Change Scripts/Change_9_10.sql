...
GO
PRINT N'Altering [dbo].[ClientComputerErrors]...';


GO
ALTER TABLE [dbo].[ClientComputerErrors]
    ADD [EnabledFeatures] NVARCHAR (2000) NULL;
...
ALTER TRIGGER [dbo].[ClientSettings_Versions_U]
	...
	BEGIN
		...
		IF (UPDATE(MenuUpdateInterval) OR UPDATE(CaptureWorkItemInterval) OR UPDATE(CaptureActiveWindowInterval) OR UPDATE(CaptureScreenShotInterval)
			...
			OR UPDATE(MouseMovingThreshold) OR UPDATE(CaptureDeadlockInMins) OR UPDATE(IsOutlookAddinRequired) OR UPDATE(IsOutlookAddinMailTrackingId))
		BEGIN
	...
	END
GO
PRINT N'Altering [dbo].[UpsertClientComputerError]...';
...
-- =============================================
ALTER PROCEDURE [dbo].[UpsertClientComputerError]
	@id int output,
