/*
Deployment script for Jobcontrol_dev
...
GO
/*
:setvar DatabaseName "Jobcontrol_dev"
:setvar DefaultFilePrefix "Jobcontrol_dev"
...
GO
PRINT N'Rename refactoring operation with key 8ca8d44b-0018-4a6e-a4d2-ac152db023d5 is skipped, element [dbo].[ClientSettings].[AutoWorkingStatusChange] (SqlSimpleColumn) will not be renamed to AutoReturnFromMeeting';
...
ALTER TABLE [dbo].[ClientSettings]
    ADD [AutoReturnFromMeeting] BIT NULL;
...
ALTER TRIGGER [dbo].[ClientSettings_Versions_U]
	...
	BEGIN
		...
		IF (UPDATE(MenuUpdateInterval) OR UPDATE(CaptureWorkItemInterval) OR UPDATE(CaptureActiveWindowInterval) OR UPDATE(CaptureScreenShotInterval)
			...
			OR UPDATE(MouseMovingThreshold) OR UPDATE(CaptureDeadlockInMins) OR UPDATE(IsOutlookAddinRequired) OR UPDATE(IsOutlookAddinMailTrackingId) OR UPDATE(ManualWorkItemEditAgeLimit) OR UPDATE(AutoReturnFromMeeting))
		BEGIN
...
END
GO
IF NOT EXISTS (SELECT OperationKey FROM [dbo].[__RefactorLog] WHERE OperationKey = '8ca8d44b-0018-4a6e-a4d2-ac152db023d5')
INSERT INTO [dbo].[__RefactorLog] (OperationKey) values ('8ca8d44b-0018-4a6e-a4d2-ac152db023d5')
