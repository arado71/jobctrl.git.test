CREATE TABLE [dbo].[ClientSettings] (
    ...
    [CreateDate]                         DATETIME       CONSTRAINT [DF_ClientSettings_CreateDate] DEFAULT (getutcdate()) NOT NULL,
    ...
    [WorkDetectorRules]                  NVARCHAR (MAX) NULL,
    [CensorRules]                        NVARCHAR (MAX) NULL,
    ...
    [MenuVersion]                        BINARY (8)     DEFAULT ((0)) NOT NULL,
    ...
    [CollectorRules]                     NVARCHAR (MAX) NULL,
    [CollectorRulesVersion]              BINARY (8)     DEFAULT ((0)) NOT NULL,
    [IsOutlookAddinRequired]             BIT            NULL, 
    [IsOutlookAddinMailTrackingId]       BIT            NULL, 
    [DefaultWorkId]                      INT            NULL, 
    [CaptureDeadlockInMins]              INT            NULL, 
    [MouseMovingThreshold]               INT            NULL, 
    [EnabledFeatures]                    NVARCHAR(2000) NULL, 
    [PluginFailThreshold]                INT            NULL, 
    [CollectedItemAggregateInMins]       INT            NULL, 
    [ForceCountdownRules]                BIT            NULL,
    [MsProjectAddress]                   NVARCHAR(200)  NULL, 
    [ManualWorkItemEditAgeLimit]         INT            NULL, 
    [AutoReturnFromMeeting]				 BIT			NULL, 
    [IsOutlookAddinMailTrackingUseSubject] BIT			NULL, 
    [TelemetryMaxCount]                  INT            NULL,
    [TelemetryMaxAgeInMins]              INT            NULL,
    [TelemetryCollectedKeys]             NVARCHAR(2000) NULL,
    [IsMeetingUploadModifications]			 BIT			NULL, 
    [IsMeetingTentativeSynced]			 BIT			NULL, 
    [IsInjectedInputAllowed]			 BIT			NULL, 
    [DataCollectionSettings] INT NULL, 
    [IsNotificationShown] INT NULL, 
    [IsTodoListEnabled] BIT NOT NULL DEFAULT 0,
    [MailTrackingSettings] INT NULL,
    [DiagnosticOperationMode] INT NULL,
    [IsGoogleCalendarTrackingEnabled]           BIT            NULL,
    [IsAnonymModeEnabled] BIT NULL, 
    [VoxIsNameMandatory] BIT NULL, 
    [VoxQuality] INT NULL, 
    [VoxIsManualStartStopEnabled] BIT NULL, 
    [DisplayOptions] INT NULL, 
    [NonWorkStateNotificationParams] NVARCHAR(50) NULL, 
    [WorkStateNotificationParams] NVARCHAR(50) NULL, 
    [TasksChangedNotificationParams] NVARCHAR(50) NULL, 
	[PasswordExpiresAt]				DATETIME	NULL,
	[AutoStartClientOnNonWorkDays]		BIT NULL,
    [MeetingPluginTaskIdSettings] INT NULL, 
    [AdHocMeetingDefaultSelectedTaskId] INT NULL, 
	[IsVideoRecordingEnabled] BIT NULL,
	[VideoRecordingFps] FLOAT NULL,
	[VideoRecordingBitRate] INT NULL,
	[IsWorkEnabledOutsideWorkTimeStartEnd] BIT NULL,
    CONSTRAINT [PK_ClientSettings] PRIMARY KEY CLUSTERED ([Id] ASC)
);


GO
CREATE UNIQUE NONCLUSTERED INDEX [IX_ClientSettings_UserId]
    ON [dbo].[ClientSettings]([UserId] ASC);
...
CREATE TRIGGER [dbo].[ClientSettings_Versions_U]
	...
	BEGIN
		...
		IF (UPDATE(MenuUpdateInterval) OR UPDATE(CaptureWorkItemInterval) OR UPDATE(CaptureActiveWindowInterval) OR UPDATE(CaptureScreenShotInterval)
			...
			OR UPDATE(IsVideoRecordingEnabled) OR UPDATE(VideoRecordingFps) OR UPDATE(VideoRecordingBitRate)
			OR UPDATE(IsWorkEnabledOutsideWorkTimeStartEnd))
		BEGIN
			...
			(
				...
				OR i.IsVideoRecordingEnabled <> d.IsVideoRecordingEnabled OR (i.IsVideoRecordingEnabled IS NOT NULL AND d.IsVideoRecordingEnabled IS NULL) OR (i.IsVideoRecordingEnabled IS NULL and d.IsVideoRecordingEnabled IS NOT NULL)
				OR i.VideoRecordingFps <> d.VideoRecordingFps OR (i.VideoRecordingFps IS NOT NULL AND d.VideoRecordingFps IS NULL) OR (i.VideoRecordingFps IS NULL and d.VideoRecordingFps IS NOT NULL)
				OR i.VideoRecordingBitRate <> d.VideoRecordingBitRate OR (i.VideoRecordingBitRate IS NOT NULL AND d.VideoRecordingBitRate IS NULL) OR (i.VideoRecordingBitRate IS NULL and d.VideoRecordingBitRate IS NOT NULL)
				OR i.IsWorkEnabledOutsideWorkTimeStartEnd <> d.IsWorkEnabledOutsideWorkTimeStartEnd OR (i.IsWorkEnabledOutsideWorkTimeStartEnd IS NOT NULL AND d.IsWorkEnabledOutsideWorkTimeStartEnd IS NULL) OR (i.IsWorkEnabledOutsideWorkTimeStartEnd IS NULL and d.IsWorkEnabledOutsideWorkTimeStartEnd IS NOT NULL)
				)
		END
	END
GO
