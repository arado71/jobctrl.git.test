...
if ERRORLEVEL 1 (
	set /p TARGETSERVER=SQL Server: 
	set /p USER=User ^(leave empty for Windows Authentication^): 
	if "!USER!" NEQ "" set /p PASSWORD=Password: 
	set /p TARGETDBNAME=Database name: 
	if "!USER!" == "" ( 
		set SQLCMDCONNECTIONPARAMS=-S !TARGETSERVER! -E -d !TARGETDBNAME!
	) else (
		set SQLCMDCONNECTIONPARAMS=-S !TARGETSERVER! -U !USER! -P !PASSWORD! -d !TARGETDBNAME!
...
for /f "tokens=3 delims=_." %%a in ("%CURRENTSCRIPTFILENAME%") do (
	@echo PRINT^("%CURRENTSCRIPTFILEPATH%"^) >> %DEPLOYSCRIPTFILE%
	@echo :r "%CURRENTSCRIPTFILEPATH%" >> %DEPLOYSCRIPTFILE%
	@echo SET QUOTED_IDENTIFIER OFF; >> %DEPLOYSCRIPTFILE%
	@echo DECLARE @currVer INT; EXEC @currVer = [dbo].[GetSchemaVersion]; IF @currVer ^<^> %%a RAISERROR^('Wrong version.',16,1^) >> %DEPLOYSCRIPTFILE%
