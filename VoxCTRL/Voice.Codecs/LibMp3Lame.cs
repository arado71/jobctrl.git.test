...
namespace VoxCTRL.Voice.Codecs
{
	...
	//http://sixfeetsix.blogspot.hu/2012/04/managed-code-to-interact-with.html
	...
	public class LibMp3Lame : IDisposable
	{
		...
		public enum MpegMode
		{
			Stereo = 0,
			JointStereo = 1,
			//DualChannel = 2, /* LAME doesn't supports this! */
			Mono = 3,
			NotSet = 4,
		}
		...
		*/
		...
		[DllImport("libmp3lame.dll", CallingConvention = CallingConvention.Cdecl)]
		static extern int lame_set_mode(IntPtr lame_global_flags, MpegMode mode);
		...
		[DllImport("libmp3lame.dll", CallingConvention = CallingConvention.Cdecl)]
		static extern int lame_set_out_samplerate(IntPtr lame_global_flags, int rateInHz);

		// int CDECL lame_set_lowpassfreq(lame_global_flags *, int);
		[DllImport("libmp3lame.dll", CallingConvention = CallingConvention.Cdecl)]
		static extern int lame_set_lowpassfreq(IntPtr lame_global_flags, int freq);
		...
		*/
		...
		// Input has L & R channel data interleaved.
		[DllImport("libmp3lame.dll", CallingConvention = CallingConvention.Cdecl)]
		...
		public void LameSetMode(MpegMode mode)
		{
		...
		{
			if (lame_set_out_samplerate(lame_global_flags, rateInHz) != 0)
				throw new LibMp3LameException("lame_set_out_samplerate returned an error");
		}

		public void LameSetLowPassFreq(int freq)
		{
			if (lame_set_lowpassfreq(lame_global_flags, freq) != 0)
				throw new LibMp3LameException("lame_set_lowpassfreq returned an error");
		}
		...
		public int LameEncodeBuffer(short[] bufferL, short[] bufferR, int nsamples, byte[] mp3Buffer)
		{
			...
			int ret = lame_encode_buffer(lame_global_flags, bufferL, bufferR, nsamples, p, mp3Buffer.Length);
			pinnedArray.Free();
			if (ret < 0)
		...
		public int LameEncodeBufferInterleaved(byte[] pcm, int nsamples, byte[] mp3Buffer)
		{
			GCHandle pinnedArray = GCHandle.Alloc(mp3Buffer, GCHandleType.Pinned);
			IntPtr p = pinnedArray.AddrOfPinnedObject();
			int ret = lame_encode_buffer_interleaved(lame_global_flags, pcm, nsamples, p, mp3Buffer.Length);
			pinnedArray.Free();
			if (ret < 0)
				throw new LibMp3LameException("lame_encode_buffer_interleaved returned an error (" + ret + ")");
			return ret;
		}
