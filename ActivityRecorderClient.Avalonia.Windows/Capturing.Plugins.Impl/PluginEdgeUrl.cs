...
{
	public class PluginEdgeUrl : ICaptureExtension
	{
		public const string PluginId = "Internal.EdgeUrl";
		...
		{
			if (!string.Equals(processName, "microsoftedge.exe", StringComparison.OrdinalIgnoreCase)) yield break;

			AppVersionLogger.LogAssemblyVersionEdge(hWnd);
			string url;
			if (UrlHelper.TryGetUrlFromWindow(hWnd, Browser.Edge, out url))
			{
