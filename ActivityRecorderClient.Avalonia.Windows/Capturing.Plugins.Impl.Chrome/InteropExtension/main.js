...
function addJcIdIfNeededMessageView(messageView) {
    ...
    threadView.getThreadIDAsync().then(function (tid) {
        var emailData = gmail.get.email_data(tid);
        var body = "";
        for (var thread in emailData.threads) {
            body = body.concat(emailData.threads[thread].content_plain);
        }
        ...
        if (res != null && res.length > 1) return null;
        gmail.get.email_source_promise(tid).then(function (rawEmailData) {
            var reference = /References: (<[^>]+>)/i.exec(rawEmailData);
            if (reference != null && reference.length > 1) {
                reference = reference[1];
            }
            if (reference != undefined && reference !== "") {
                console.log("generating jcid from References: " + reference);
                id = sha256(reference);
            } else {
                var inReplyTo = /In-Reply-To: (<[^>]+>)/i.exec(rawEmailData);
                if (inReplyTo != null && inReplyTo.length > 1) {
                    inReplyTo = inReplyTo[1];
                }
                if (inReplyTo != undefined && inReplyTo !== "") {
                    console.log("generating jcid from In-Reply-To: " + inReplyTo);
                    id = sha256(inReplyTo);
                } else {
                    var messageId = /Message-ID: (<[^>]+>)/i.exec(rawEmailData);
