...
namespace Tct.ActivityRecorderClient.Notification
{
	public class NotificationWinService : INotificationService
	{
		...
		public static void SetInactiveTopMost(System.Windows.Forms.IWin32Window frm)
		{
		...
		{
			return (DialogResult)TopMostMessageBox.Show(message);
		}

		public DialogResult ShowMessageBox(string message, string title)
		{
			return (DialogResult)TopMostMessageBox.Show(message, title);
		}

		public DialogResult ShowMessageBox(string message, string title, MessageBoxButtons buttons)
		{
			return (DialogResult)TopMostMessageBox.Show(message, title, (System.Windows.Forms.MessageBoxButtons)buttons);
		}

		public DialogResult ShowMessageBox(string message, string title, MessageBoxButtons buttons, MessageBoxIcon icon)
		{
			return (DialogResult)TopMostMessageBox.Show(message, title, (System.Windows.Forms.MessageBoxButtons)buttons, (System.Windows.Forms.MessageBoxIcon)icon);
		}
		...
		{
			return (DialogResult)PasswordExpiredMessageBox.Instance.ShowDialog(true);
		}
	}
}