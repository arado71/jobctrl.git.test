	...
	public partial class PreferencesForm : FixedMetroForm, ILocalizableControl
	{
		...
		private static void BindKeyCode(DataGridViewComboBoxColumn cb)
		{
			List<KeyValuePair<string, Keys>> listToBind =
				HotkeySetting.ValidKeys.Select(n => new KeyValuePair<string, Keys>(HotkeySetting.GetNameForKey(n), (Keys)n)).ToList();
			cb.DisplayMember = "Key";
			cb.ValueMember = "Value";
			cb.DataSource = listToBind;
		}
		...
		{
			if (hks.KeyCode == Forms.Keys.None) throw new ValidationException(Labels.Hotkeys_EmptyHotkeyError);

			if (!HotkeySetting.ValidKeys.Contains(hks.KeyCode)) throw new ValidationException(Labels.Hotkeys_InvalidKeyValue);
			if (hks.IsInRange(Forms.Keys.F1, Forms.Keys.F12) == false && hks.HasModifier == false)
		...
		private void HandleHotkeyCreateClicked(object sender, EventArgs e)
		{
			...
			List<Forms.Keys> usedKeys = hotkeySettings.Select(hk => hk.KeyCode).ToList();
            Forms.Keys newUnusedKey = HotkeySetting.ValidKeys.FirstOrDefault(k => usedKeys.Contains(k) == false);
