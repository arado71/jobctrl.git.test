...
namespace System.Diagnostics
{
	public static class DebugEx
	{
		...
		public static bool IsInDesignMode
		{
			get
			{
				bool isInDesignMode = ComponentModel.LicenseManager.UsageMode == ComponentModel.LicenseUsageMode.Designtime || Debugger.IsAttached == true || Design.IsDesignMode;
		...
		public static void EnsureGuiThread()
		{
			...
			if (!guiThreadId.HasValue) log.Fatal("No Set was called " + Environment.NewLine + new StackTrace(true));
			if (guiThreadId != Thread.CurrentThread.ManagedThreadId) log.Fatal("Method not called from a GUI thread " + Environment.NewLine + new StackTrace(true));
		...
		public static void EnsureBgThread()
		{
			...
			if (!guiThreadId.HasValue) log.Fatal("No Set was called " + Environment.NewLine + new StackTrace(true));
			if (guiThreadId == Thread.CurrentThread.ManagedThreadId) log.Fatal("Method not called from a BG thread " + Environment.NewLine + new StackTrace(true));
			Debug.Assert(guiThreadId.HasValue, "No Set was called");
