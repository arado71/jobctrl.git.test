	...
	public class Censor
	{
		...
		private void HideWindow(DesktopWindow desktopWindow, DesktopCapture desktopCapture)
		{
			...
			foreach (var screen in desktopCapture.Screens)
			{
				...
				using var canvas = new SKCanvas(screen.OriginalScreenImage);
				var paint = new SKPaint { Color = SKColors.Black, Style = SKPaintStyle.Fill };
				canvas.DrawRegion(area, paint);
			}
		}

		private static SKRegion GetWindowAreaForScreen(Rectangle rectangle, DesktopWindow desktopWindow, DesktopCapture desktopCapture)
		{
			var result = new SKRegion(TransformOrigin(desktopWindow.WindowRect, rectangle.Location));
			foreach (var window in desktopCapture.DesktopWindows.TakeWhile(n => n != desktopWindow))
			{
				result.Op(TransformOrigin(window.WindowRect, rectangle.Location), SKRegionOperation.Difference);
			}
			result.Op(TransformOrigin(rectangle, rectangle.Location), SKRegionOperation.Intersect);
			return result;
		}

		private static SKRectI TransformOrigin(Rectangle original, System.Drawing.Point negVector)
		{
			var result = original;
			result.Offset(-negVector.X, -negVector.Y);
			return result.ToRectI();
		}
	}
}
