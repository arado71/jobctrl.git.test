...
namespace Tct.ActivityRecorderClient.Capturing.Core
{
	...
	public class CaptureCoordinator : IDisposable
	{
		...
		public ClientMenuLookup CurrentMenuLookupUnsafe { get => currentMenuLookup; }
		...
		public void Start()
		{
			...
			isStarting = false;
			if (CurrentMenuLookup.ClientMenu?.Works != null)
			{
				// not sure why this was not raised before, but make sure we raise the current menu on start
				ThreadPool.QueueUserWorkItem(_ => MenuManagerCurrentMenuChanged(this,
					new MenuEventArgs(CurrentMenuLookupUnsafe.ClientMenu, CurrentMenuLookupUnsafe.ClientMenu, CurrentMenuLookupUnsafe))
				);
			}
		}
		...
		public void Stop()
		{
			...
			ActivityRecorderClientWrapper.StartShutdown();
			...
			{
				log.Debug("Disconnect from server failed", exception);
			}
