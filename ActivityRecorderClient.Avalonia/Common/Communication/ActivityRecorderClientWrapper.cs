	...
	public class ActivityRecorderClientWrapper : IDisposable
	{
		...
		public static void Execute(Action<ActivityRecorderServiceReference.ActivityRecorderClient> action, int timeout = DefaultTimeout, EndpointConfiguration endpointConfig = null)
		{
			using (var wrapper = new ActivityRecorderClientWrapper(endpointConfig, timeout))
			{
				try
				{
					var callTask = Task.Run(() =>  action(wrapper.Client));
					int winner = Task.WaitAny(callTask, shutdownSignal.Task);

					if (winner == 1)
					{
						wrapper.Client.Abort();
						throw new OperationCanceledException("Shutdown requested during WCF call.");
					}
		...
		public static T Execute<T>(Func<ActivityRecorderServiceReference.ActivityRecorderClient, T> command, int timeout = DefaultTimeout, EndpointConfiguration endpointConfig = null)
		{
			using (var wrapper = new ActivityRecorderClientWrapper(endpointConfig, timeout))
			{
				try
				{
					var callTask = Task.Run(() => command(wrapper.Client));
					int winner = Task.WaitAny(callTask, shutdownSignal.Task);
					...
					return callTask.GetAwaiter().GetResult();
				}
				catch (CommunicationException ex)
				{
					wrapper.CloseIfUnusable(ex);
		...
		private static TaskCompletionSource shutdownSignal = new TaskCompletionSource();
		private static int shutdownGracePeriod = (int)TimeSpan.FromSeconds(3).TotalMilliseconds;
		public static void StartShutdown()
		{
			Task.Run(async () =>
			{
				await Task.Delay(shutdownGracePeriod);
				log.Info("Force closing communication");
				WcfExceptionLogger.Shutdown();
				sharedPool.Dispose();
				shutdownSignal.SetResult();
			});
		}
