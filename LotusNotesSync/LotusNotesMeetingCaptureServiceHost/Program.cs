	...
	static class Program
	{
		...
		static void Main(string[] args)
		{
			...
			try
			{
				...
				{
					log.Error("LotusNotesSync process must be started with exactly one argument.");
					return;
				}
				int parentPid;
				if (!int.TryParse(args[0], out parentPid))
				{
					log.Error("LotusNotesSync process must be started with exactly one argument that is the PID of the parent process.");
					return;
				}
				...
				Application.Run(new LotusNotesMeetingCaptureHostForm(parentPid));
			}
			catch (Exception ex)
			{
				log.Error("Unable to start LotusNotesSync process", ex);
				throw;
			}
		}
