	...
	static class ProgramWin
	{
		...
		static void Main(string[] args)
		{
			...
			try
			{
				...
				var initialized = updateSerivce.Initialize();
				if (!initialized)
				{
			...
			catch (Exception ex)
			{
				...
				if (ex.Message == UpdateClickOnceWinService.InitializeExMsgNotStartedFromMenu)
				{
					MessageBox.Show(Labels.Program_NotificationNonClickOnceStartBody, Labels.Program_NotificationNonClickOnceStartTitle);
				}
				else if (ex.Message == UpdateWixWinService.InitializeExMsgMissingBootstrap)
				{
			...
			try
			{
				...
				try
				{
					singletonMutex = new Mutex(true, "Local\\" + ConfigManager.MutexName, out owned, mSec); //Global would prevent proper usage on terminal servers
