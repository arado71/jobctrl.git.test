...
{
#if AppConfigNISZTest || DEBUG

	public class AppConfigNISZTest : AppConfig
	{
		protected override EndpointAddress ServiceNetTcpEndpointAddress => new EndpointAddress(
			new Uri("net.tcp://test.jobctrl.nisz.hu:9000/ActivityRecorderService", UriKind.Absolute),
		...
		protected override EndpointAddress ServiceHttpsEndpointAddress => new EndpointAddress(
			new Uri("https://test.jobctrl.nisz.hu/JobCTRL/", UriKind.Absolute)
		);

		protected override EndpointAddress ActiveDirectoryNetTcpEndpointAddress => new EndpointAddress(
			new Uri("net.tcp://test.jobctrl.nisz.hu:9000/ActiveDirectoryLoginService", UriKind.Absolute),
			EndpointIdentity.CreateX509CertificateIdentity(defaultCertificate)
		);

		protected override EndpointAddress ActiveDirectoryHttpsEndpointAddress => new EndpointAddress(
			new Uri("https://test.jobctrl.nisz.hu/ActiveDirectoryLoginService", UriKind.Absolute)
		);
		...
		public override string WebsiteUrl => "https://test.jobctrl.nisz.hu/";
		public override string AppClassifier => "NISZ Test";
