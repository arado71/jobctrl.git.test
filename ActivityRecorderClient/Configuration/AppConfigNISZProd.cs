...
{
#if AppConfigNISZProd || DEBUG

	public class AppConfigNISZProd : AppConfig
	{
		protected override EndpointAddress ServiceNetTcpEndpointAddress => new EndpointAddress(
			new Uri("net.tcp://prod.jobctrl.nisz.hu:9000/ActivityRecorderService", UriKind.Absolute),
		...
		protected override EndpointAddress ServiceHttpsEndpointAddress => new EndpointAddress(
			new Uri("https://prod.jobctrl.nisz.hu/JobCTRL/", UriKind.Absolute)
		);

		protected override EndpointAddress ActiveDirectoryNetTcpEndpointAddress => new EndpointAddress(
			new Uri("net.tcp://prod.jobctrl.nisz.hu:9000/ActiveDirectoryLoginService", UriKind.Absolute),
			EndpointIdentity.CreateX509CertificateIdentity(defaultCertificate)
		);

		protected override EndpointAddress ActiveDirectoryHttpsEndpointAddress => new EndpointAddress(
			new Uri("https://prod.jobctrl.nisz.hu/ActiveDirectoryLoginService", UriKind.Absolute)
		);
		...
		public override string WebsiteUrl => "https://prod.jobctrl.nisz.hu/";
		public override string AppClassifier => "NISZ";
