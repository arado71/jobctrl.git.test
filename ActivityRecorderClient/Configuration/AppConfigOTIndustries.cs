...
{
#if AppConfigOTIndustries || DEBUG

	public class AppConfigOTIndustries : AppConfig
	{
		protected override EndpointAddress ServiceNetTcpEndpointAddress => new EndpointAddress(
			new Uri("net.tcp://jobctrl.olajterv.hu:9000/ActivityRecorderService", UriKind.Absolute),
		...
		protected override EndpointAddress ServiceHttpsEndpointAddress => new EndpointAddress(
			new Uri("https://jobctrl.olajterv.hu/JobCTRL/", UriKind.Absolute)
		);

		protected override EndpointAddress ActiveDirectoryNetTcpEndpointAddress => new EndpointAddress(
			new Uri("net.tcp://jobctrl.olajterv.hu:9000/ActiveDirectoryLoginService", UriKind.Absolute),
			EndpointIdentity.CreateUpnIdentity(@"_jobctrlapp@jobctrl.olajterv.hu")
		);

		protected override EndpointAddress ActiveDirectoryHttpsEndpointAddress => new EndpointAddress(
			new Uri("https://jobctrl.olajterv.hu/ActiveDirectoryLoginService", UriKind.Absolute)
			, EndpointIdentity.CreateUpnIdentity(@"_jobctrlapp@jobctrl.olajterv.hu")
		);

		public override string WebsiteUrl => "https://jobctrl.olajterv.hu/";
		...
		public override string AppClassifier => "OTIndustries";
