...
{
#if AppConfigAldi || DEBUG

	public class AppConfigAldi : AppConfig
	{
		protected override EndpointAddress ServiceNetTcpEndpointAddress => new EndpointAddress(
			new Uri("net.tcp://jobctrl.test.aldi-sued.com:9000/ActivityRecorderService", UriKind.Absolute),
		...
		protected override EndpointAddress ServiceHttpsEndpointAddress => new EndpointAddress(
			new Uri("https://jobctrl.test.aldi-sued.com/JobCTRL/", UriKind.Absolute)
		);

		protected override EndpointAddress ActiveDirectoryNetTcpEndpointAddress => new EndpointAddress(
			new Uri("net.tcp://jobctrl.test.aldi-sued.com:9000/ActiveDirectoryLoginService", UriKind.Absolute),
			EndpointIdentity.CreateX509CertificateIdentity(defaultCertificate)
		);

		protected override EndpointAddress ActiveDirectoryHttpsEndpointAddress => new EndpointAddress(
			new Uri("https://jobctrl.test.aldi-sued.com/ActiveDirectoryLoginService", UriKind.Absolute)
		);
		...
		public override string WebsiteUrl => "https://jobctrl.test.aldi-sued.com/";
		public override string AppClassifier => "Aldi";
