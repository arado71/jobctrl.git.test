...
{
#if AppConfigFundamenta || DEBUG

	public class AppConfigFundamenta : AppConfig
	{
		protected override EndpointAddress ServiceNetTcpEndpointAddress => new EndpointAddress(
			new Uri("net.tcp://jobctrl.fundamenta.local:9000/ActivityRecorderService", UriKind.Absolute),
		...
		protected override EndpointAddress ServiceHttpsEndpointAddress => new EndpointAddress(
			new Uri("https://backoffice-jobctrl.fundamenta.local/JobCTRL/", UriKind.Absolute)
		);

		protected override EndpointAddress ActiveDirectoryNetTcpEndpointAddress => new EndpointAddress(
			new Uri("net.tcp://jobctrl.fundamenta.local:9000/ActiveDirectoryLoginService", UriKind.Absolute),
			EndpointIdentity.CreateX509CertificateIdentity(defaultCertificate)
		);

		protected override EndpointAddress ActiveDirectoryHttpsEndpointAddress => new EndpointAddress(
			new Uri("https://backoffice-jobctrl.fundamenta.local/ActiveDirectoryLoginService", UriKind.Absolute)
		);

		public override string WebsiteUrl => "https://jobctrl.fundamenta.local/";
		...
		public override string AppClassifier => "Fundamenta";
