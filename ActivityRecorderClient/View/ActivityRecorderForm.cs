...
using Tct.JcMon.IAccessibility;
#if OcrPlugin
...
namespace Tct.ActivityRecorderClient.View
{
	...
	public partial class ActivityRecorderForm : Form, ILocalizableControl
	{
		...
		public ActivityRecorderForm(ApplicationStartType startType)
		{
			...
			((Platform.PlatformWinFactory)Platform.Factory).MainForm = this; //hax for external dependency for RuleManagementWinService
		...
		{ 
...
#endif
			miExit.Enabled = !AppControlServiceHelper.IsRegistered;
			...
			{
				var hks = MigrateLegacyHotkey(captureCoordinator.WorkDetectorHotKey.Value, HotkeyActionType.NewWorkDetectorRule);
			...
			{
				var hks = MigrateLegacyHotkey(ConfigManager.LocalSettingsForUser.HotKey.Value, HotkeyActionType.ResumeOrStopWork);
			...
			{
				var hks = MigrateLegacyHotkey(ConfigManager.LocalSettingsForUser.ManualMeetingHotKey.Value, HotkeyActionType.StartManualMeeting);
			...
			if (ConfigManager.LocalSettingsForUser.ClearAutoRuleTimersHotKey.HasValue && ConfigManager.LocalSettingsForUser.ClearAutoRuleTimersHotKey.Value != Keys.None)
			{
				var hks = MigrateLegacyHotkey(ConfigManager.LocalSettingsForUser.ClearAutoRuleTimersHotKey.Value, HotkeyActionType.ClearAutoRuleTimer);
				ConfigManager.LocalSettingsForUser.ClearAutoRuleTimersHotKey = Keys.None;
			...
			guiThreadWatchdogManager.MissingReset += (_, __) =>
			{
				try
				{
					...
					var stackTrace = new StackTrace(watchedThread, false);
		...
		private void GoogleCredentialManagerOnReportResult(object sender, SingleValueEventArgs<bool> e)
		{
			context.Post(_ =>
			{
				if (e.Value)
					...
					notificationService.ShowMessageBox(Labels.GoogleCredetialManager_ReportFailedText, Labels.GoogleCredetialManager_ReportTitle, MessageBoxButtons.OK, MessageBoxIcon.Error);
			}, null);
		}
		...
		private void ActiveDirectoryAuthManagerOnUserIdChanged(object sender, EventArgs eventArgs)
		{
			context.Post(_ =>
			{
				notificationService.ShowMessageBox(Labels.UserIdMappingChangedBody, Labels.UserIdMappingChangedTitle, MessageBoxButtons.OK);
			...
			context.Post(_ =>
							{
								...
								notificationService.ShowNotification("UPD_NOT", TimeSpan.Zero, Labels.AutoUpdate_UpdateAvailable,
										...
										() =>
										{
											...
											Labels.ConfirmExitStillWorkingTitle, MessageBoxButtons.OKCancel);
				...
				context.Post(_ =>
							{
								...
								try
								{
									notificationService.ShowMessageBox(Labels.Login_NotificationPasswordErrorBody, Labels.Login_NotificationPasswordErrorTitle, MessageBoxButtons.OK, MessageBoxIcon.Error);
			...
			switch (type)
			{
				...
				case HotkeyActionType.StartManualMeeting:
					...
					if (ConfigManager.MaxManualMeetingInterval >= 0)
						...
						notificationService.ShowMessageBox(Labels.AddMeeting_DisabledManualMeetingWarningBody, Labels.AddMeeting_DisabledManualMeetingWarningTitle, MessageBoxButtons.OK);
					break;
		...
		private void SystemEventsService_SessionSwitch(object sender, Microsoft.Win32.SessionSwitchEventArgs e)
		{
		...
		{
			var result = notificationService.ShowMessageBox(Labels.ConfirmRestartDueElevatedModeBody, Labels.ConfirmRestartDueElevatedModeTitle, MessageBoxButtons.OKCancel);
			if (result != DialogResult.OK) return;
