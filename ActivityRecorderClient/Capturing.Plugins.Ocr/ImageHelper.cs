...
namespace Tct.ActivityRecorderClient.Capturing.Plugins.Ocr
{
	public static class ImageHelper
	{
		...
		public static Bitmap ScaledCopy(this Bitmap sourceImage, double scale, float brightness = 1.0f, float contrast = 1.0f, InterpolationMode interpolation = InterpolationMode.HighQualityBicubic)
		{
			return ScaledCopy(sourceImage, (int)(sourceImage.Width * scale), (int)(sourceImage.Height * scale), brightness, contrast, interpolation);
		}
		...
		public static Bitmap Crop(this Bitmap sourceImage, Rectangle area)
		{
			if (sourceImage.Width >= area.X + area.Width && sourceImage.Height >= area.Y + area.Height)
			{
				return sourceImage.Clone(area, sourceImage.PixelFormat);
			}
		...
		public static Bitmap ScaledCopy(this Bitmap sourceImage, int newWidth, int newHeight, float brightness = 1.0f, float contrast = 1.0f, InterpolationMode interpolation = InterpolationMode.HighQualityBicubic)
		{
			var result = new Bitmap(newWidth, newHeight);
			using (Graphics g = Graphics.FromImage(result))
			{
				g.InterpolationMode = interpolation;
				g.DrawImage(sourceImage, new Rectangle(0, 0, newWidth, newHeight), 0, 0, sourceImage.Width, sourceImage.Height, GraphicsUnit.Pixel, GetBrightnessContrastAttributes(brightness, contrast));
			}
