...
namespace Tct.ActivityRecorderClient.Controller
{
	...
	public class CurrentWorkController : CurrentWorkControllerBase, IMutualWorkTypeService
	{
		...
		private void CheckIfAutomaticRuleMode()
		{
			if (parent.HasAllMatchingRule)
			{
				if (!isQuestionDisplayed)
				{
					isQuestionDisplayed = true;
					switch (ConfigManager.AccessLevel)
					{
						case UserAccessLevel.Reg:
							if (notificationService.ShowMessageBox(Labels.AutoMode_QuerySwitchOff, Labels.AutoMode_Title, System.Windows.Forms.MessageBoxButtons.OKCancel) == System.Windows.Forms.DialogResult.OK)
							{
								try
								{
									var success = ActivityRecorderClientWrapper.Execute(c => c.SwitchAutomaticRules(ConfigManager.UserId));
									log.Debug($"SwitchAutomaticRules returned with {success}");
									if (success)
										parent.RestartClientRulesManager();
								}
								catch(Exception ex)
								{
									log.Error("SwitchAutomaticRules failed", ex);
								}
							}
							break;
						case UserAccessLevel.Adm:
							if (notificationService.ShowMessageBox(Labels.AutoMode_QueryDisplaySettings, Labels.AutoMode_Title, System.Windows.Forms.MessageBoxButtons.OKCancel) == System.Windows.Forms.DialogResult.OK)
							{
								try
								{
									RecentUrlQuery.Instance.OpenLink(ConfigManager.WebsiteUrlFormatString + "DictionaryAutoRules.aspx");
								}
								catch (Exception ex)
								{
									log.Error("Showing DictionaryAutoRules failed", ex);
								}
							}
							break;
						default:
							notificationService.ShowNotification(NotificationKeys.TaskSwitchNotPossible, TimeSpan.FromMinutes(1), Labels.AutoMode_Title, Labels.AutoMode_NotificationText);
							break;
					}
