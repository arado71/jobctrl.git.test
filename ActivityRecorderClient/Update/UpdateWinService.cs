...
namespace Tct.ActivityRecorderClient.Update
{
	public class UpdateWinService : IUpdateService
	{
		...
		private UpdateWinService() //we need to get CurrentVersion before running Initialize, also we have to avoid circular depenencies between cctors (ConfigManager, ElevatedPrivilegesHelper, IsolatedStorageSerializationHelper)
		{
			try
			{
				//todo resolve this ugly circular depenency (probably by introducing new property IUpdateService.IsAvailable ? but I don't like that either)
				//We can call UpdateClickOnceWinService Initialize() several times and it will yield the same result that is not a problem.
				//But we cannot call UpdateWixWinService.Initialize() here because this is called from ConfigManager's cctor before setting IsAppLevelStorageNeeded
				//and UpdateWixWinService.Initialize() will use IsolatedStorageSerializationHelper which will use that IsAppLevelStorageNeeded value
				inner = new UpdateClickOnceWinService();
				if (!inner.Initialize())
				{
