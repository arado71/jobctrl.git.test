...
namespace Tct.ActivityRecorderClient.Capturing.Desktop
{
	public abstract class DesktopCaptureService : IDesktopCaptureService
	{
		...
		protected abstract Bitmap GetScreenShot(Rectangle screenBounds);
		...
		private void SystemEventsServiceSessionSwitch(object sender, SessionSwitchEventArgs e)
		{
			switch (e.Reason)
			{
		...
		{
			using (var regionVisibleScreen = new Region(Rectangle.Empty))
			{
				foreach (var screen in screens)
				{
					regionVisibleScreen.Union(screen.Bounds);
				}
				//windows are in Z-order
				foreach (var windowInfo in windows)
				{
					windowInfo.ClientArea = windowInfo.ClientRect.Width * windowInfo.ClientRect.Height;
					if (windowInfo.ClientArea > 0)
					{
						using (var regionClient = new Region(windowInfo.ClientRect))
						{
							regionClient.Intersect(regionVisibleScreen);
							windowInfo.VisibleClientArea = GetArea(regionClient);
							regionVisibleScreen.Exclude(windowInfo.WindowRect);
						}
					}
				}
			}
		...
		private static int GetArea(Region region)
		{
			...
			{
				var scans = region.GetRegionScans(matrix);
				var area = 0f;
		...
		protected static byte[] EncodeImage(Image screenShot, out string extension)
		{
			long quality = ConfigManager.JpegQuality;
			...
			using (EncoderParameter myEncoderParameter = new EncoderParameter(Encoder.Quality, quality))
			{
				...
				using (var img = ScaleByPercent(screenShot, scale))
		...
		public static Image ScaleByPercent(Image imgPhoto, float nPercent)
		{
			const int sourceX = 0;
			const int sourceY = 0;
			...
			var result = new Bitmap(destWidth, destHeight, PixelFormat.Format24bppRgb);
			//SetResolution throws on mono and probably unnecessary on win
			//result.SetResolution(imgPhoto.HorizontalResolution, imgPhoto.VerticalResolution);

			using (Graphics grPhoto = Graphics.FromImage(result))
			{
