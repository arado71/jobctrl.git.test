...
namespace Tct.ActivityRecorderClient.Taskbar
{
	//code based on Windows API Code Pack for Microsoft .NET Framework
	class TaskbarWin7Service
	{
		...
		internal static ITaskbarList4 Instance
		{
			get
			{
				if (taskbarList == null && ConfigManager.IsWindows7)
				{
					lock (sharedLock)
					{
						if (taskbarList == null)
		...
		public TaskbarWin7Service(IntPtr hWnd)
		{
			this.hWnd = hWnd;
		}

		public void SetProgressValue(UInt64 ullCompleted, UInt64 ullTotal)
		{
			if (Instance == null) return;
			Instance.SetProgressValue(hWnd, ullCompleted, ullTotal);
		}

		public void SetProgressState(TaskbarProgressBarStatus tbpFlags)
		{
			if (Instance == null) return;
			Instance.SetProgressState(hWnd, tbpFlags);
		}
