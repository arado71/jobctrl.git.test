...
namespace Tct.ActivityRecorderClient.Menu
{
	public class MenuMacBuilder
	{
		...
		private SimpleWorkTimeStats TotalWorkTimeStats { get; set; }
		...
		public void UpdateMenu(ClientMenu menu)
		{
			...
			{
				firstIdx = (int)nsMenu.IndexOf(PlaceHolder);
				if (firstIdx > -1)
				{
			...
			{
				nsMenu.InsertItem(newMenu[j], firstIdx);
			}
			if (newMenu.Count == 0)
			{
				nsMenu.InsertItem(PlaceHolder, firstIdx);
			}
		}
			...
			using (var fakeParent = new NSMenuItem()) //create a fake parent to store the items in menu
			{
				...
				foreach (var item in menu)
				{
					if (item.IsSeparatorItem)
						continue;
					FlattenItemAtIdx(fakeParent, item, (int)fakeParent.Submenu.IndexOf(item), factor);
				}

				menu.Clear();
				menu.AddRange(fakeParent.Submenu.Items);
		...
		private int FlattenItemAtIdx(NSMenuItem parent, NSMenuItem child, int childIdxInParent, int factor)
		{
			...
			for (int i = (int)child.Submenu.Count - 1; i >= 0; i--)
			{
				var grandChild = child.Submenu.ItemAt(i);
				...
				parent.Submenu.InsertItem(grandChild, childIdxInParent + 1); //we go backwards so the order is the same (inserting here would also remove from child's DropDownItems if it still had an owner).
				inserted++;
			}
		...
		private void AddTopMenuItemsIfFirstTime(int firstIdx)
		{
			...
			if (placeIdx <= -1)
				return;
			nsMenu.InsertItem(NSMenuItem.SeparatorItem, ++placeIdx);
			nsMenu.InsertItem(topPriority, ++placeIdx);
			nsMenu.InsertItem(topEndDate, ++placeIdx);
			nsMenu.InsertItem(topTargetWorkTime, ++placeIdx);
			firstTime = false;
		}
		...
		{
			var currentControls = parentItem.Submenu.Items.OfType<NSMenuItemWithWorkData>().ToList();
			if (!topItems.Select(n => n.WorkData.Id.Value).SequenceEqual(currentControls.Select(n => (int)n.Tag)))
			{
